...
- (id)initWithFrame:(CGRect)frame andController:(ATMHud *)c {
    if ((self = [super initWithFrame:frame])) {
		...
		backgroundLayer.backgroundColor = [UIColor colorWithWhite:0.12 alpha:p.alpha].CGColor;
...
- (void)calculate {
	if (!caption || [caption isEqualToString:@""]) {
		...
		CGSize captionSize = [caption sizeWithFont:[UIFont boldSystemFontOfSize:14] constrainedToSize:CGSizeMake(160, 200) lineBreakMode:NSLineBreakByCharWrapping];
		...
		if (fixedSize.width > 0 & fixedSize.height > 0) {
			...
			hasFixedSize = YES;
			captionSize = [caption sizeWithFont:[UIFont boldSystemFontOfSize:14] constrainedToSize:CGSizeMake(s.width-p.margin*2, 200) lineBreakMode:NSLineBreakByCharWrapping];
		...
		if (!hasFixedSize) {
			if (p.accessoryPosition == ATMHudAccessoryPositionTop || p.accessoryPosition == ATMHudAccessoryPositionBottom) {
				if (progress > 0) {
					...
					if (captionSize.width+p.margin*2 < progressRect.size.width) {
						captionSize = [caption sizeWithFont:[UIFont boldSystemFontOfSize:14] constrainedToSize:CGSizeMake(progressRect.size.width, 200) lineBreakMode:NSLineBreakByCharWrapping];
						captionRect.size = captionSize;
			...
			if (p.accessoryPosition == ATMHudAccessoryPositionTop || p.accessoryPosition == ATMHudAccessoryPositionBottom) {
				...
				if (marginX < p.margin) {
					captionSize = [caption sizeWithFont:[UIFont boldSystemFontOfSize:14] constrainedToSize:CGSizeMake(160, 200) lineBreakMode:NSLineBreakByCharWrapping];
					captionRect.size = captionSize;
...
- (CGSize)calculateSizeForQueueItem:(ATMHudQueueItem *)item {
	...
	if (!item.caption || [item.caption isEqualToString:@""]) {
		...
		BOOL hasFixedSize = NO;
		CGSize captionSize = [item.caption sizeWithFont:[UIFont boldSystemFontOfSize:14] constrainedToSize:CGSizeMake(160, 200) lineBreakMode:NSLineBreakByCharWrapping];
		...
		if (!hasFixedSize) {
			...
			if (item.accessoryPosition == ATMHudAccessoryPositionTop || item.accessoryPosition == ATMHudAccessoryPositionBottom) {
				...
				marginX = 0.5*deltaWidth;
				if (marginX < p.margin) {
					captionSize = [item.caption sizeWithFont:[UIFont boldSystemFontOfSize:14] constrainedToSize:CGSizeMake(160, 200) lineBreakMode:NSLineBreakByCharWrapping];
...
- (void)applyWithMode:(ATMHudApplyMode)mode completion:(void(^)())completion {
	switch (mode) {
...
- (void)show {
	[self calculate];
	[self applyWithMode:ATMHudApplyModeShow completion:nil];
}

- (void)hide {
   [self applyWithMode:ATMHudApplyModeHide completion:nil];
}

- (void)hideWithCompletion:(void (^)())completion {
	[self applyWithMode:ATMHudApplyModeHide completion:completion];
}

- (void)update {
	[self calculate];
	[self applyWithMode:ATMHudApplyModeUpdate completion:nil];
}
